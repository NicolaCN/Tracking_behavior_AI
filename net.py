...
# version with SUM aggregation function
class LSTMModel3b(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, device):
        super(LSTMModel3b, self).__init__()
...
class LSTMModel3c(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, device):
        super(LSTMModel3c, self).__init__()
        ...
        self.relu1 = nn.ReLU(inplace=False)
        self.sp = nn.Sigmoid()
        #self.fc_tmp = nn.Linear(self.directions*self.directions*hidden_dim*parameters.seq_dim, self.directions*self.directions*hidden_dim)
        self.fc = nn.Linear(self.directions*self.directions*hidden_dim, output_dim)
        self.to(device)
    ...
    def forward(self, x):
        ...
        #out = torch.logsumexp(out, dim=1)   # BEST
        #out = torch.reshape(out, (out.shape[0], out.shape[1]*out.shape[2]))  # solution FD: too heavy
        #out = self.fc_tmp(out)                                               # solution FD: too heavy
        out = self.fc(out)  # ORIGINAL
        #out = self.sp(self.fc(out))  # NEW
        #out = 1.0-torch.exp(-5*self.fc(out))
        #out = torch.log(1+1.5*self.relu1(self.fc(out)))
